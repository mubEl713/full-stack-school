...
const AccountingDashboardPage = async () => {
  const [payments, income, expenses, feeStructures] = await Promise.all([
    prisma.feePayment.aggregate({ _sum: { amount: true } }),
    prisma.income.aggregate({ _sum: { amount: true } }),
    prisma.expense.aggregate({ _sum: { amount: true } }),
    prisma.feeStructure.findMany({
      include: {
        grade: { include: { students: { select: { id: true } } } },
  ...
  const totalIncome =
    (payments._sum.amount || 0) + (income._sum.amount || 0);
  const totalExpense = expenses._sum.amount || 0;
  const netBalance = totalIncome - totalExpense;

  const outstanding = feeStructures.reduce((sum, fs) => {
    const expected = fs.amount * fs.grade.students.length;
    const collected = fs.payments.reduce((s, p) => s + p.amount, 0);
  ...
  return (
    <div className="p-4 flex flex-col gap-4">
      <div className="flex gap-4 flex-wrap">
        ...
        />
        <StatCard label="Outstanding Fees" value={fmt(outstanding)} color="bg-lamaYellowLight" />
      </div>
