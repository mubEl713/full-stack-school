...
const IncomeListPage = async ({
  ...
  if (queryParams?.search) {
    query.source = { contains: queryParams.search, mode: "insensitive" };
  }
  ...
  const renderRow = (item: Income) => (
    ...
    >
      <td className="flex items-center gap-4 p-4">{item.source}</td>
      ...
      <td>
        <div className="flex items-center gap-2">
          {canWrite && (
            <>
              <FormContainer table="income" type="update" data={item} />
              <FormContainer table="income" type="delete" id={item.id} />
            </>
          )}
        </div>
      </td>
  ...
  return (
    <div className="bg-white p-4 rounded-md flex-1 m-4 mt-0">
      <div className="flex items-center justify-between">
        <h1 className="hidden md:block text-lg font-semibold">Incomes</h1>
        <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
          ...
          <div className="flex items-center gap-4 self-end">
            ...
            {canWrite && <FormContainer table="income" type="create" />}
          </div>
        </div>
      </div>
